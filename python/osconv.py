...
full_len = 2 * npoints
ff = np.fft.ifft((np.fft.fft(h, full_len)) * np.fft.fft(s_n, full_len))
...
plt.plot(tf - delay_v, cf, label='full convolve', linestyle='-.')
#  plt.plot(tf[:len(cv)] + delay_v, cv, label='valid', linestyle=':')
#  plt.plot(tf[:len(cs)], cs, label='same')
plt.plot(t_ff - delay_v, np.real(ff), label='fft oneshot', color='magenta', marker='o', linestyle='None')
#  plt.plot(t_ff, (np.real(ff)), label='ov fft', color='magenta')
plt.plot(tsfo + delay_v, np.real(sfo), label='overlap save', marker='+', linestyle='None')
plt.legend()
plt.show()
