...
int channelizer(const int downsamp, const int n_full, const int n_filt, const float samp_rate) {
  ...
  fftwf_complex *full_out = fftwf_alloc_complex(n_out);
  ...
  fftwf_complex *fft_in = fftwf_alloc_complex(n_fft_h);
  fftwf_complex *fft_filt = fftwf_alloc_complex(n_fft_h);
  ...
  poly_filt_design_f(n_filt, f_cutoff, samp_rate, &filt[0], filt_full, n_cols, downsamp, filt_c, fft_filt);
  ...
  fftwf_plan pinv = fftwf_plan_many_dft_c2r(inv_c.rank, inv_c.n_size, inv_c.howmany, fft_mult,
  ...
  for (size_t idx = 0; idx < n_loops; idx++) {  // TODO Get right number of buffers
    ...
    if (n_full - in_start < n_buffer) {
      ...
      fftwf_execute_dft_r2c(psig, buffer_in, fft_in);
    } else {
      fftwf_execute_dft_r2c(psig, &full_in[in_start], fft_in);
    }
